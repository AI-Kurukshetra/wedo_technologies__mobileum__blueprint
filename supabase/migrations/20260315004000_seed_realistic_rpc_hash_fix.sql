-- TeleGuard Pro: avoid digest() signature issues; generate unique hashes via pgcrypto
...
begin
  ...
  jwt_role := coalesce(auth.role(), claims->>'role', current_setting('request.jwt.claim.role', true), '');
  ...
  enriched as (
    select
      ...
      -- Unique per row; avoids digest() signature issues entirely.
      (encode(gen_random_bytes(16), 'hex') || '-' || n::text) as source_row_hash,
      call_start_at,
