-- TeleGuard Pro: RPC wrapper to run realistic demo seed without direct DB access
-- Purpose: allow `SUPABASE_SERVICE_ROLE_KEY` callers to run the set-based seed safely via PostgREST RPC.
-- Security: only allows execution when JWT role is `service_role`.
...
begin
  ...
  -- Ensure all existing users are members (so any login can see data)
  ...
  -- Create rule + version (required FK for alerts)
  ...
  -- CDRs (50k) using set-based insert
  ...
  enriched as (
    select
      ...
      encode(digest(concat_ws('|', org_id, import_id, call_start_at::text, duration_seconds::text, destination_country, n::text), 'sha256'), 'hex') as source_row_hash,
      call_start_at,
